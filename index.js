...
function showMainMenu(ctx, lang) {
    const menus = {
        kk: [
            ['📚 Қандай білім беру бағыттары бар?'],
...
bot.hears(['📚 Қандай білім беру бағыттары бар?', '📚 Образовательные программы', '📚 Study Programs'], (ctx) => {
    ctx.reply("Қандай білім беру бағыттары бар? Университетте: • Педагогикалық ғылымдар; • Өнер және гуманитарлық ғылымдар; • Әлеуметтік ғылымдар, журналистика және ақпараттандыру; • Бизнес, басқару және құқық; • Жаратылыстану ғылымдар, математика және статистика; • Ақпараттық - комуникациялық технологиялар
• Инженерлік, өңдеу және құрылыс салалары;  • Ауылшаруашылық және биоресустары; • Қызмет көрсету");
});
