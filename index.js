...
bot.on('text', async (ctx) => {
  ...
  if (answer) {
    ...
    ctx.reply("*** Я не могу ответить на этот вопрос, так как он не содержит конкретной информации о НАО 'Атырауский университет имени Халела Досмухамедова' *** Мен бұл сұраққа жауап бере алмаймын, өйткені онда 'Халел Досмұхамедов атындағы Атырау университеті' КЕАҚ туралы нақты ақпарат жоқ ***");
    try {
      const aiAnswer = await askGPT(userText);
      ctx.reply(aiAnswer);
    } catch (err) {
      ctx.reply("*** Пожалуйста, перефразируйте свой вопрос или предоставьте больше деталей *** Сұрағыңызды қайталаңыз немесе қосымша мәліметтер беріңіз ***");
      console.error(err);
    }
  }
});
